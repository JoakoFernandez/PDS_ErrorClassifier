...
class CacheService {
    constructor() {
        ...
        this.client.on('error', (err) => {
            this.isConnected = false;
            logger_1.logger.warn('Redis connection error — falling back to AI classification', { error: err.message });
        });
    ...
    async connect() {
        try {
            ...
            logger_1.logger.warn('Could not connect to Redis on startup', { error: err.message });
        }
    }
    async disconnect() {
    ...
    buildKey(errorCode) {
        return `${config_1.config.redis.keyPrefix}${errorCode.toUpperCase()}`;
    }
    /**
    ...
    async get(errorCode) {
        ...
        try {
            ...
            logger_1.logger.debug('Cache hit', { errorCode, cachedAt: entry.cachedAt });
            return entry.classification;
        }
        catch (err) {
            logger_1.logger.warn('Cache read error', { errorCode, error: err.message });
            return null;
        }
    }
    /**
     ...
     */
    async set(errorCode, classification) {
        if (!this.isConnected)
            return false;
        try {
            const key = this.buildKey(errorCode);
            ...
            await this.client.setex(key, config_1.config.redis.ttlSeconds, JSON.stringify(entry));
            logger_1.logger.debug('Cached classification', { errorCode, ttl: config_1.config.redis.ttlSeconds });
            return true;
        }
        catch (err) {
            logger_1.logger.warn('Cache write error', { errorCode, error: err.message });
            return false;
        }
    }
    /**
     ...
     */
    async invalidate(errorCode) {
        if (!this.isConnected)
            return false;
        try {
            const key = this.buildKey(errorCode);
            await this.client.del(key);
            logger_1.logger.info('Cache invalidated', { errorCode });
            return true;
        }
        catch (err) {
            logger_1.logger.warn('Cache invalidation error', { errorCode, error: err.message });
            return false;
        }
    }
